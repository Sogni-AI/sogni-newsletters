# Creativity enters the agentic era

---

March / April 2026 Edition

## Meet the new Creative Assistant

Agentic AI-assisted creation is now live in Sogni Studio, Sogni Pocket, and Sogni Creative Agent.

The Creative Assistant is where **Sogni Intelligence** starts changing the feel of the product: less hunting through settings, more telling Sogni what you want to make and letting the assistant assemble the right workflow.

Choose the creation style that fits the moment:

- **Agent mode** — chat-first creation. Tell it what you want and it picks models, writes prompts, and runs multi-step workflows for you.
- **Guided mode** — chat with controls. The assistant helps, you decide.
- **Manual mode** — the classic Sogni workflow, untouched.

It handles image generation, editing, video and music creation, prompt writing, model selection, settings, media analysis, and multi-step flows — and understands the Sogni UI, Supernet, assets, and system logic, while respecting your safety filter settings. You bring the idea; it helps with the rest.

## Sogni Intelligence & chat.sogni.ai

The AI layer behind Sogni Creative Agent, SuperApps, SDKs, and APIs.

**Sogni Creative Agent** is live at [chat.sogni.ai](https://chat.sogni.ai): a familiar chat interface where replies can call Sogni tools to create and edit images, video, audio, and beyond.

Introduced in March, **Sogni Intelligence** is already moving fast. The new **Qwen3.6-35B-A3B** model (**qwen3.6-35b-a3b-gguf-iq4xs** for API users) brings stronger reasoning, tool calling, and a roughly **264k-token context window** — enough working memory for long creative briefs, multi-step plans, and rich media analysis.

In practice, Sogni apps get smarter, easier to use, more personalized, more powerful, and easier to connect with other tools. In **Sogni Photobooth**, you can describe the booth you want instead of browsing dozens of preset categories — try ideas like “put me on famous magazine covers” or “turn me into famous paintings.” In **Sogni 360**, the assistant can analyze your trip photos and choose smoother object-to-object transitions across a sequence.

Developers can plug in through OpenAI Chat Completions-compatible tools including **Open WebUI**, **OpenClaw**, **Hermes Agent**, the OpenAI SDK, and agentic coding clients that support custom OpenAI-compatible endpoints — all powered by Sogni's decentralized compute network.

[Read the docs →](https://docs.sogni.ai/sogni-intelligence/introduction)

Photobooth in action — one prompt, eight magazine-cover styles.

## Introducing Personas

Save the people in your life. Use them forever.

Personas lets you set an image and profile for yourself, your partner, your kids, your friends, your pets — once they're in, you can mention them by name and they show up. In the photo. In the video. In the right voice.

Define how they should sound in **LTX-2.3 audio**, add backstory the assistant uses when it builds scripts. It's the difference between generating *a person* and generating **Mark**.

## Sogni Restore

Bring old memories back to life.

Sogni Restore is a new iOS app for AI-powered photo restoration and enhancement, powered by Sogni Intelligence. It brings old, damaged, faded, or poorly scanned photos back to life — preserving the original character of each image without ever making it look over-edited.

- **Fix what time damaged.** Scratches, dust, stains, folds, fading, yellowing, and scan imperfections — all addressed automatically.
- **Improve quality, not character.** Sharpness and color balance get a boost while the photo still looks like itself.
- **Turn stills into motion.** Convert your restored photos into short cinematic videos, or generate new visual variations.
- **Guided by an AI Assistant.** The built-in assistant helps you choose the right tools and make edits intuitively.
- **Project-friendly.** Edit history, before-and-after comparison, organized projects, and export of images or videos.

## Leaderboards & Season 8

Season 7 wrapped and Season 8 is underway for its six-week run.

**Refreshed airdrop distribution:**

The Ambassador leaderboard was new in Season 7, continues in Season 8, and now carries one of the biggest slices of each six-week community rewards pool.

Sogni Ambassador Leaderboard

### Share Sogni. Earn from the season.

The Ambassador Leaderboard allocates **25% of each six-week community rewards pool** to users who are promoting Sogni and helping credit card purchases grow.

Share your referral link from any of our **10+ Sogni SuperApps**. Friends who sign up with your link get **25 bonus credits**, and after their first purchase, you earn **25 credits** too.

Your leaderboard share is based on purchases made by you, your referrals, and the people they refer. It becomes especially powerful with upcoming monthly subscription plans, where renewals can continue crediting the user and referrers.

[How the program works →](https://docs.sogni.ai/run-a-worker/rewards/referral-program-sogni-ambassador-rewards/)

## From Cannes with creativity

EthCC[9], in person.

We had an incredible time with you at **EthCC[9] in Cannes**. Between learning, conversations, and shared ideas, we created some truly unique visuals together.

A mix of education, creativity, and real connection — exactly what this community is about. Thanks for being part of it.

## Made by the community

Two reels worth your time. Just a small compilation — there's a lot more on our [Discord](https://discord.com/invite/2JjzA2zrrc).

Our community continues to surprise us with the range of work being created with Sogni. Each month brings new ideas, visual styles, and creative experiments that show how far these tools can be pushed.

Creator of the Month

### Hadiprasetyo

This month we're spotlighting **Hadiprasetyo** for bringing a distinct style, strong consistency, and creative energy to the Sogni community. The work stands out not just for its quality, but for the clear sense of direction behind it.

[See Hadiprasetyo's work on X →](https://x.com/hadiprasety0001/status/2036192754771148855?t=4-9HkQ5BqpoHbVy_UI2aOw&s=19)

## New Sogni LLM Worker

Run a node, power Sogni Intelligence, earn.

Powering all the new agentic LLM features in Sogni Intelligence is the new **Sogni LLM Worker** — and anyone with 24GB VRAM or more can operate a node and earn.

Earnings are based on input/output tokens generated, with pricing around **~$0.30 per 1M input tokens** and **~$0.90 per 1M output tokens**.

To install: make sure you have a recent Fast Worker installer and re-run the install script:

→ [Windows installer](https://api.sogni.ai/v1/download/worker/windows) → [Linux installer](https://api.sogni.ai/v1/download/worker/linux)

[Worker docs →](https://docs.sogni.ai/run-a-worker/fast-worker/about-sogni-fast-worker/)

## What's coming in Season 8

Six weeks of expansion — across product, personalization, and AI-powered experiences:

- **Unlimited Plan Subscriptions** rolling out
- **Sogni Photobooth** for Android and iOS — native apps continuing the rollout
- **Agentic AI** getting dedicated product focus and paid marketing support
- **Sogni LoRA training** — train your own personalization
- **Seedance 2.0** on the way

## Learn with Sogni

Four techniques worth a closer look this month.

Architecture · Tutorial

### From sketch to finished vision

A rough architectural sketch can turn into something real in minutes — materials, colors, structure, landscaping, atmosphere. With **Qwen Image Edit** and **Flux 2** inside Sogni, edits move fast. With **Start & End Frame** in LTX or WAN, you can turn that first sketch into a transition a buyer instantly connects with.

Tools · Tutorial

### PaintOver, when prompts aren't enough

PaintOver lets you guide image generation more directly. Instead of relying only on prompts, paint over specific parts of an image to show exactly what you want to change, add, or refine. The AI regenerates only that area, based on your input. A more precise way to iterate, adjust details, and build the result step by step.

Photobooth · Tutorial

### Motion transfer, no prompt required

In Sogni Photobooth, you can create motion transfer without writing a prompt. Use the default settings and get started — no complicated setup, no steep learning curve. Built to keep things simple, so you can focus on creating and becoming whoever you want to be.

SuperApps · Tutorial

### Give your product a new dimension

Turn simple product images into polished motion visuals built to stand out. In Sogni SuperApps, format, motion, and atmosphere can turn a static image into something people notice — including gentle animation that makes products pop.

[Start creating →](https://app.sogni.ai/create)

157M+ Creations · 89,564 Users · 1,087 GPUs

## Your Next Steps

- **Chat:** Try the new brain at [chat.sogni.ai](https://chat.sogni.ai).
- **Create:** Open [Sogni Pocket](https://www.sogni.ai/pocket) and let the Creative Assistant do the heavy lifting.
- **Build:** Plug Qwen3.6 into your own apps via the [Sogni Intelligence API](https://docs.sogni.ai/sogni-intelligence/introduction).

Keep creating. Keep amplifying what makes you human.

Team Sogni
